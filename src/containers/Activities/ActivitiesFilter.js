...
import { getActivities, getAllSubSectors } from 'redux/actions';
import { SelectInput, TextInput } from 'components';
import { withTranslation } from '../../../i18n';

const ActivitiesFilter = ({ onGetActivities, onGetSubSectors, subSectors, t }) => {
  ...
  const handleFilter = (values) => {
    onGetActivities({ filters: values });
    setIsOpen(false);
  };
    ...
    <>
      <div>
        <span className="rounded-full shadow-sm">
          <button
            type="button"
            className="inline-flex justify-center w-full rounded-md border border-gray-100 px-4 py-2 bg-white text-sm leading-5 font-medium text-gray-700 hover:bg-gray-100 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-50 active:text-gray-800 transition ease-in-out duration-150"
            id="options-menu"
...
const mapDispatchToProps = (dispatch) => ({
  onGetActivities: (query) => dispatch(getActivities(query)),
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withTranslation('common')(ActivitiesFilter));
