...
const SectorListItem = ({ data, handleClick, handleDelete, t }) => (
  <tr onClick={handleClick}>
    ...
    <td className="px-6 py-4 whitespace-nowrap">
      <div className="text-sm leading-5 font-medium truncate text-gray-600">
        {data.scopeName && data.scopeName.includes('{') ? (
          <div className={`${!JSON.parse(data.scopeName)[i18n.language] && 'text-gray-400'}`}>
            {JSON.parse(data.scopeName)[i18n.language] ||
...
export default withTranslation('common')(SectorListItem);
